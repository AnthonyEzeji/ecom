...
async function getUserOrders(){
    await axios.get(`http://3.87.187.44:5000/orders/${JSON.parse(window.sessionStorage.getItem('session'))._id}`).then(res=>{setOrders(res.data.reverse())})
}
getUserOrders()
}, [])
useEffect(() => {
...
async function getProducts(){
    ...
    await axios.get(`http://3.87.187.44:5000/products/${tempArr[i]}`).then(res=>{
